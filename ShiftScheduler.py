...
class ShiftScheduler:
    def __init__(self):
        ...
        # スタッフごとの重みペナルティ、デフォルト50として辞書を作成
    ...
    def set_data(self, staff_df, calendar_df, staff_penalty, staff_ng_date, off_penalty):
        # リストの設定
    ...
    def build_model(self):
        ...
        ### 目的関数とスラック変数の定義 ###
        # 各スタッフの勤務希望日数の不足数と超過数を最小化する(重み付けあり)
        self.model += pulp.lpSum([self.S2penalty_weight[s] * (self.y_under[s] + self.y_over[s]) for s in self.S] + [self.penalty_off * self.z_over[s] for s in self.S]
        )
